...
export function* saveCost() {
    ...
    try {
      ...
      objectCost.price = costForm?.price.slice(2).replace(/\./g, "").replace(/,/g, ".") * 100;
      objectCost.goal = costForm?.goal;

      if (costForm.type == "Feed") {
        const extra = { 'brand':  costForm?.brand, 'weight': costForm?.weight.replace(/,/g, ".") };
